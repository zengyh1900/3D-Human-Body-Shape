...
class HumanShapeAnalysisDemo(QtGui.QMainWindow):
    ...
    def __init__(self, file):
        ...
        if createButtons:
            ...
            self.connect(self.reset_button, QtCore.SIGNAL('clicked()'), self.reset)
            ...
            self.connect(self.pre_button, QtCore.SIGNAL('clicked()'), self.showDialog)
            ...
            for i in range(0, self.viewer3D.data.measure_num):
                ...
                slider.setStatusTip('%d. %s' % (i, self.viewer3D.data.measure_str[i]))
                slider.setRange(-100, 100)
                slider.valueChangeForwarded.connect(self.viewer3D.sliderForwardedValueChangeHandler)
        ...
        try:
            ...
            for i in range(2, len(self.editList)):
                try:
                    ...
                    data.append(0)
            [t_data, value] = self.viewer3D.predict(np.array(data).reshape(len(data), 1))
            for i in range(2, len(self.editList)):
                self.editList[i].setText("%f" % (value[i, 0] / 10))
            for i in range(0, len(self.slider)):
                self.slider[i].valueChangeForwarded.disconnect(self.viewer3D.sliderForwardedValueChangeHandler)
                self.slider[i].setValue(t_data[i] / 3.0 * 100.0)
                self.slider[i].valueChangeForwarded.connect(self.viewer3D.sliderForwardedValueChangeHandler)
        except ValueError:
