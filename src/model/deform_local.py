...
class deformLocal:
    ...
    def local_matrix(self):
        ...
        start = time.time()
        L_file = self.data.NPYpath + \
            ("localMapper/L%d.npy" % (self.data.paras['mapping_version']))
        if self.data.paras['reload_L']:
            L_list = []
            L_tosave = []
            for i in range(0, self.data.face_num):
                print('  calculating local mapping with face: %d' % i)
                S = np.array(self.data.o_deform[
                             i * 9:i * 9 + 9, :]).reshape(9, self.data.body_count)
                S = S.transpose()
                ...
                measures.shape = (
                    measures.size / self.data.body_count, self.data.body_count)
            ...
            print(' [**] finish solve local transformation in %fs' %
                  (time.time() - start))
            return L_list
        else:
            print(' [**] finish solve local transformation in %fs' %
    ...
    def local_rebuild(self):
        ...
        for i in range(0, self.data.body_count):
            print('rebuilding deform_local-based_v%d: %d  ...' %
                  (self.data.paras['mapping_version'], i))
