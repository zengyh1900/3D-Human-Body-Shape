...
class deformModel:
    ...
    def mapping(self, weight):
        ...
        weight = self.data.d_pca_mean[:self.demo_num, :] + weight * self.data.d_pca_std[:self.demo_num,:]
        basis = self.data.d_basis[:, :self.data.d_basis_num]
        deformation = self.data.mean_deform + np.matmul(basis, weight) * self.data.std_deform
