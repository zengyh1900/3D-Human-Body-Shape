...
class Masker:
    ...
    def __init__(self, data):
        ...
        [self.p2m, self.p2p, self.p2f, self.m2p, self.m2f, self.m2v, self.mask] = self.getMap()
    ...
    def getColor(self):
        ...
        if self.data.paras['reload_color']:
            ...
            for i in range(0, self.data.vertex_num):
                ...
                color_list.append((body_part[i, 0], body_part[i, 1], body_part[i, 2]))
            color_set = [item for item in tmp]
            self.data.save_NPY([color_list_path, color_set_path], [color_list, color_set])
        else:
            [list_tmp, set_tmp] = self.data.load_NPY([color_list_path, color_set_path])
            color_set = []
            for i in range(0, set_tmp.shape[0]):
                color_set.append((set_tmp[i,0], set_tmp[i,1], set_tmp[i,2]))
            color_list = []
            for i in range(0, list_tmp.shape[0]):
                color_list.append((list_tmp[i,0], list_tmp[i,1], list_tmp[i,2]))
    ...
    def getMap(self):
        ...
        if self.data.paras['reload_mask']:
            ...
            self.data.save_NPY([p2f_path, m2f_path, m2v_path, mask_path],[p2f, m2f, m2v, mask])
        else:
            [p2f, m2f, m2v, mask] = self.data.load_NPY([p2f_path, m2f_path, m2v_path, mask_path])
        print (' [**] finish loading color data.')
            ...
            for i in range(0, self.data.measure_num):
                ...
                for j in part:
                    ...
                    for k in range(0, len(self.p2f[j])):
                        ...
                        vertex_set.add(self.data.o_faces[self.p2f[j][k] * 3, 0] - 1)
                        vertex_set.add(self.data.o_faces[self.p2f[j][k] * 3 + 1, 0] - 1)
                        vertex_set.add(self.data.o_faces[self.p2f[j][k] * 3 + 2, 0] - 1)
