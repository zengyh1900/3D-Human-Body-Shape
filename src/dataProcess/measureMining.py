...
class measureMining:
    ...
    def __init__(self, data):
        ...
        self.test_size = 300
        self.data = data
        self.GIRTH = np.array(self.data.paras["GIRTH"], dtype=bool).reshape(self.data.measure_num, 1)
        self.LENGTH = np.array(self.data.paras["LENGTH"], dtype=bool).reshape(self.data.measure_num, 1)
        ...
        self.impute_name = ["SimpleFill", "KNN", "MICE"]#, "SoftImpute",   "Nuclear", "Matrix" ]#, "Similarity"]
        self.impute_method = [SimpleFill(), KNN(), MICE()]#,
                              # SoftImpute(5), NuclearNormMinimization(),
                              # MatrixFactorization(learning_rate=0.01), SimilarityWeightedAveraging()]
    ...
    def get_measure_basis(self):
        ...
        if self.data.paras["reload_measure_basis"]:
            ...
            m_basis, m_sigma, M = np.linalg.svd(self.data.t_measures, full_matrices=0)
            ...
            m_coeff = np.array(m_coeff).reshape(m_coeff.shape[0], self.data.body_count)
        ...
        if self.data.paras['reload_cfTable']:
            ...
            for i in range(0, self.data.measure_num):
                ...
                for j in range(i+1, self.data.measure_num):
                    a = np.array(self.data.t_measures[i,self.test_size:]).reshape(1, self.data.body_count-self.test_size)
                    b = np.array(self.data.t_measures[j,self.test_size:]).reshape(self.data.body_count-self.test_size, 1)
                    similar = (a.dot(b)/(np.linalg.norm(a)*np.linalg.norm(b)))[0,0]
            ...
            for i in range(0, top):
                ...
                ws.cell(row=i*(top+2)+1, column=2).value = self.data.measure_str[i]
                for j in range(0, top):
                    ws.cell(row=i*(top+2)+j+2, column=1).value = self.data.measure_str[j]
                    ws.cell(row=i*(top+2)+j+2, column=2).value = table[i, j]
            wb.save(filename=self.data.paras['ansPath']+'similarity.xlsx')
            self.data.save_NPY([self.filePath+"cfTable.npy"], [table])
            print (' [**] finish get and save cf of measures in %s s.'%(time.time()-start))
            return table
        else:
            print (' [**] finish get and save cf of measures in %s s.' % (time.time() - start))
    ...
    def simpleAverage(self, flag, data, dist):
        ...
        for j in range(0, output.shape[0]):
            if output[j, 0] == 0:
                coeff = np.array(self.cfTable[j, :]).reshape(self.data.measure_num, 1)
                if (abs(coeff).sum() == 0):
                    output[j,0] = 0
                else:
    ...
    def spetialTest(self):
        ...
        for i in range(0, len(test_data)):
            ...
            for j in range(0, self.data.measure_num):
                output_ws.cell(row=(i*table_size)+3+j, column=1).value = self.data.measure_str[j]
            output_ws.cell(row=(i * table_size) + 22, column=1).value = 'MEAN GIRTH'
            output_ws.cell(row=(i * table_size) + 23, column=1).value = 'MEAN LENGTH'
            ...
            for m in range(0, len(self.impute_method)):
                output_ws.cell(row=(i * table_size) + 2, column=k).value = self.impute_name[m]
                ...
                error = np.array(np.mean(abs(output - input), axis=1)).reshape(self.data.measure_num, 1)
                print(error)
                # error = error * self.data.std_measures
                for j in range(0, self.data.measure_num):
                    output_ws.cell(row=(i * table_size) + 3 + j, column=k).value = error[j, 0]
                output_ws.cell(row=(i * table_size) + 22, column=k).value = np.average(abs(error[(~test_data[i]) & self.GIRTH]))
                output_ws.cell(row=(i * table_size) + 23, column=k).value = np.average(abs(error[(~test_data[i]) & self.LENGTH]))
                output_ws.cell(row=(i * table_size) + 24, column=k).value = np.average(abs(error[(~test_data[i])]))
                k = k + 1
        ...
        for i in range(0, len(test_data)):
            ...
            for m in range(0, len(self.impute_method)):
                ...
                for j in range(0, self.test_size):
                    print('Experiment %d, '%i, self.impute_name[m], ' sample: ', j)
                    input = np.array(self.data.t_measures[:, j]).reshape(self.data.measure_num, 1)
                    output = self.Imputate(test_data[i], input, False, self.impute_method[m])
            ...
            output_ws.cell(row=(i * table_size) + 2, column=k).value = 'simple weighted'
            error = np.zeros((self.data.measure_num, 1))
            for j in range(0, self.test_size):
                input = np.array(self.data.t_measures[:, j]).reshape(self.data.measure_num, 1)
            ...
            output_ws.cell(row=(i * table_size) + 2, column=k).value = 'case amplication'
            error = np.zeros((self.data.measure_num, 1))
            for j in range(0, self.test_size):
                input = np.array(self.data.t_measures[:, j]).reshape(self.data.measure_num, 1)
