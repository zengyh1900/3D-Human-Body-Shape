...
class rawData:
    ...
    def __init__(self, filename):
        ...
        # self.paras = json.loads(open(filename, "rb").read())
        self.rawDataPath = self.paras['dataPath']+"NPYdata/rawData/"
        self.dataSetPath = self.paras['dataPath']+"female/"

        self.measure_num = self.paras['measure_num']
        self.body_count = self.paras['body_count']
        ...
        # load all data
        self.o_normals = np.load(open(self.rawDataPath+"Normals.npy", 'rb'))
        [self.o_file_list, self.o_faces,self.o_vertex,self.t_vertex,self.mean_vertex,self.std_vertex]=\
            self.load_vertex_data()
        [self.o_measures, self.t_measures, self.mean_measures, self.std_measures] = \
    ...
    def load_vertex_data(self):
        ...
        if self.paras['reload_vertex_data']:
            ...
            o_faces = np.array(list(map(int, str.replace('\n', ' ').replace('f', '').split( ))))
            print (o_faces.shape)
            ...
            print (' [**] finish load_vertex_data from %s in %fs' % (self.rawDataPath, time.time() - start))
    ...
    def load_measure_data(self):
        ...
        if True:#self.paras['reload_measure_data']:
            value = open(self.rawDataPath+'body_measures.txt', 'r').read().split()
            o_measures = []
            ...
            print (' [**] finish load_measure_data from %s in %fs' % (self.rawDataPath, time.time() - start))
    ...
    def load_control_point(self):
        ...
        if self.paras['reload_cp']:
            ...
            print (' [**] finish load_control_point from %s in %fs' % (self.rawDataPath, time.time() - start))
    ...
    def ComputeNormals(self):
        ...
        for face in self.o_faces:
            AB = np.array(self.o_vertex[face[0]]) - np.array(self.o_vertex[face[1]])
            AC = np.array(self.o_vertex[face[0]]) - np.array(self.o_vertex[face[2]])
            n = np.cross(AB, -AC)
            n /= np.linalg.norm(n)
            for i in range(0, 3):
