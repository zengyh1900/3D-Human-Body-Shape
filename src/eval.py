...
def rebuild(idx):
    ...
    data = t_measure[:, idx].reshape(reshaper.M_NUM, 1)
    [vertex, n, f] = body.mapping(data, flag=flag)
    reshaper.save_obj(os.path.join(ANS_DIR, "%s_%d.obj"%(label, idx)), vertex, f + 1)

    data = mean_measure + data * std_measure
    output = np.array(reshaper.calc_measure(cp, vertex, facet))
    error = output - data
...
def run(label="female"):
    ...
    for i in range(0, reshaper.M_NUM):
        ws.cell(row=1, column=i + 2).value = reshaper.M_STR[i]
    ...
    body_num = t_measure.shape[1]
    ans = np.array(results).reshape(body_num, M_NUM).transpose()
